...
namespace KDTree_UnitTest
{
    ...
    /// <summary>
    ///This is a test class for BoundingSphereTest and is intended
    ///to contain all BoundingSphereTest Unit Tests
    ///</summary>
	[TestClass()]
	public class BoundingSphereTest
	{
		...
		public void IntersectsTest()
		{
			...
			bool expected = true;
			bool actual = target.Intersects(aabb, intersection_type);
			...
			{	// Test1: Solid intersections (IE: Sphere and AABB can reside inside of each other and it counts as an intersection)
				...
				expected = true;
				actual = target.Intersects(aabb, intersection_type);
				Assert.AreEqual(expected, actual);
				...
				target = new BoundingSphere(new Vector3(6, 6, 6), (aabb.MaxCorner - new Vector3(6, 6, 6)).Length() + slight_offset);	//Sphere crosses max corner of AABB
				expected = true;
				actual = target.Intersects(aabb, intersection_type);
				Assert.AreEqual(expected, actual);

				target = new BoundingSphere(new Vector3(6, 6, 6), (aabb.MaxCorner - new Vector3(6, 6, 6)).Length() - slight_offset);	//Sphere does not cross max corner of AABB
				expected = false;
				actual = target.Intersects(aabb, intersection_type);
				Assert.AreEqual(expected, actual);
			}
